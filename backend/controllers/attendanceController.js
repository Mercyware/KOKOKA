...
// @desc    Generate QR code session for attendance
// @route   POST /api/attendance/qr-session
// @access  Private (Teacher/Admin)
exports.createQRSession = asyncHandler(async (req, res) => {
  const {
    classId,
    period = 'FULL_DAY',
    expiresIn = 3600,
  ...
  const sessionData = {
    ...
    signature: crypto.createHash('sha256')
      .update(`${sessionId}-${classId}-${req.school.id}-${Date.now()}`)
      .digest('hex')
  };

  // Store session in Redis with expiration
  await redisClient.setEx(`qr_session:${sessionId}`, expiresIn, JSON.stringify(sessionData));
  ...
  try {
    ...
    const qrPayload = {
      sessionId,
      type: 'attendance',
      v: '2.0' // version for backwards compatibility
    };
    ...
    res.status(201).json({
      ...
      data: {
        ...
        classInfo: {
          ...
          name: classExists.name,
          grade: classExists.grade,
          totalStudents: classExists.students.length
        },
        sessionInfo: {
          period,
        ...
        instructions: requireLocation
          ? `Students must be within ${locationRadius}m to scan and mark attendance`
          : 'Students should scan this QR code to mark their attendance'
      },
      message: 'QR attendance session created successfully'
    });
  } catch (error) {
    // Clean up Redis session if QR generation fails
...
// @route   GET /api/attendance/qr-session/:sessionId
// @access  Private (Teacher/Admin)
exports.getQRSessionStatus = asyncHandler(async (req, res) => {
  const { sessionId } = req.params;

  const sessionData = await redisClient.get(`qr_session:${sessionId}`);
  ...
  // Get current scan count from attendance records
  const attendanceDate = new Date();
  attendanceDate.setHours(0, 0, 0, 0);

  const scanCount = await prisma.attendance.count({
    where: {
  ...
  res.status(200).json({
    ...
    data: {
      ...
      classId: session.classId,
      period: session.period,
      isActive: session.isActive && new Date() < new Date(session.expiresAt),
...
// @desc    Generate QR code for attendance (Legacy support)
// @route   GET /api/attendance/qr-code/:classId
// @access  Private (Teacher/Admin)
exports.generateQRCode = asyncHandler(async (req, res) => {
  // Legacy endpoint - redirect to new session-based system
  const { classId } = req.params;
  const { period = 'FULL_DAY', expiresIn = 3600 } = req.query;
  ...
  await exports.createQRSession({ ...req, ...sessionReq }, res);
});
...
// @access  Private (Student)
exports.scanQRAttendance = asyncHandler(async (req, res) => {
  const { qrData, location, gpsCoordinates } = req.body;
  ...
  try {
    ...
    // Handle both new session-based and legacy QR codes
    let sessionData;
    if (parsedQRData.sessionId) {
      // New session-based QR code
      const sessionJson = await redisClient.get(`qr_session:${parsedQRData.sessionId}`);
      if (!sessionJson) {
        return res.status(400).json({
          success: false,
          message: 'QR session has expired or is invalid'
        });
      }
      sessionData = JSON.parse(sessionJson);
    } else {
      // Legacy QR code format
      sessionData = parsedQRData;
    }
    ...
    if (sessionData.schoolId !== req.school.id) {
      return res.status(400).json({
        success: false,
    ...
    if (sessionData.requireLocation && gpsCoordinates) {
      ...
      if (distance > sessionData.locationRadius) {
        return res.status(400).json({
          success: false,
          message: `You must be within ${sessionData.locationRadius}m of the classroom to mark attendance`
        });
      }
    }
    ...
    if (student.currentClassId !== sessionData.classId) {
      return res.status(400).json({
        success: false,
    ...
    // Check scan limits
    if (sessionData.maxScans) {
      const currentScans = await prisma.attendance.count({
        where: {
          classId: sessionData.classId,
          method: 'QR_CODE',
          date: {
      ...
      if (currentScans >= sessionData.maxScans) {
        return res.status(400).json({
          success: false,
    ...
    const attendance = await prisma.attendance.create({
      data: {
        ...
        date: attendanceDate,
        method: 'QR_CODE',
        location: gpsCoordinates || location,
    ...
    // Update session scan count in Redis if it's a session-based QR
    if (parsedQRData.sessionId) {
      sessionData.scanCount = (sessionData.scanCount || 0) + 1;
      await redisClient.setEx(
        `qr_session:${parsedQRData.sessionId}`,
...
module.exports = {
  ...
  createQRSession: exports.createQRSession,
  getQRSessionStatus: exports.getQRSessionStatus,
