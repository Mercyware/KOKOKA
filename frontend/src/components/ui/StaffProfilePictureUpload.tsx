...
export function StaffProfilePictureUpload({
  staffId,
  currentImageUrl,
  staffName,
  onUploadSuccess,
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/staff/${staffId}/profile-picture`, {
        method: 'POST',
        body: formData,
        credentials: 'include',
        headers,
      ...
      if (result.success) {
        ...
        onUploadSuccess?.(result.data.file.url || result.data.staff.profilePictureUrl);
      } else {
          ...
          >
            <Avatar className={cn(currentSize.avatar, 'border-2 border-dashed border-gray-300 transition-colors group-hover:border-primary')}>
              ...
              <AvatarFallback className="bg-gray-100 text-gray-600">
                {hasImage ? <Camera className="h-6 w-6" /> : getInitials(staffName)}
              </AvatarFallback>
            </Avatar>
