...
const AttendanceReports: React.FC = () => {
  ...
  const generateQRCode = async (classId: string) => {
    ...
    try {
      const response = await api.post(`/attendance/qr-session`, {
        classId,
        expiresInMinutes: 30
      });
        ...
        qrWindow?.document.write(`
          <html>
            ...
            <body style="text-align:center; padding:20px;">
              ...
              <p>${data.classInfo?.name || 'Selected Class'}</p>
              <div style="background: white; padding: 20px; display: inline-block; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                <canvas id="qrcode"></canvas>
              </div>
              <p style="color:#666; font-size:12px; margin-top: 20px;">Session ID: ${data.sessionId}</p>
              <p style="color:#666; font-size:12px;">Expires: ${new Date(data.expiresAt).toLocaleString()}</p>
              <script src="https://cdn.jsdelivr.net/npm/qrcode@1.5.3/build/qrcode.min.js"></script>
              <script>
                QRCode.toCanvas(document.getElementById('qrcode'), '${data.qrUrl}', { width: 256 });
              </script>
            </body>
          </html>
        `);
    ...
    <Layout>
      <div className="space-y-6 p-6">
        ...
        <div className="flex space-x-3">
          <Button intent="action" onClick={() => window.location.href = '/attendance/take'}>
            <UserCheck className="h-4 w-4 mr-2" />
            Take Attendance
          </Button>
          <Button intent="secondary" onClick={() => generateQRCode(selectedClass || '')}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <Button className="w-full justify-start" intent="secondary">
                ...
                <Button
                  className="w-full justify-start"
                  intent="secondary"
                  onClick={() => generateQRCode(selectedClass || '')}
                >
                  <QrCode className="h-4 w-4 mr-2" />
                  Generate QR Codes
                </Button>
